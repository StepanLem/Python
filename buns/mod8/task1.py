class Transport:
    ...
    def coordinates(self, coordinates):
        if not isinstance(coordinates, tuple) or len(coordinates) != 2 or not all(
                isinstance(coord, int) for coord in coordinates):
            raise ValueError("Invalid coordinates. Should be a tuple of two integers.")
        self.__coordinates = coordinates
    ...
    def speed(self, speed):
        if not isinstance(speed, int):
            raise ValueError("Invalid speed. Should be an integer.")
        self.__speed = speed
...
seaplane = Seaplane([5,6],6,"asd",5,5,5,"asd")
print(seaplane)
